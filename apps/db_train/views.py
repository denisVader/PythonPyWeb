...
class TrainView(View):
    def get(self, request):
        ...
        self.answer2 = Author.objects.aggregate(entry=Max('entries'))  # TODO Какой автор имеет наибольшее количество опубликованных статей?
        ...
        self.answer6 = AuthorProfile.objects.filter(stage__range=(1, 5)).annotate(username=F('author__username')) #  Какие авторы имеют стаж от 1 до 5 лет?

        self.answer7 = Author.objects.aggregate(max_age=Max('age'))  #  Какой автор имеет наибольший возраст?

        self.answer8 = Author.objects.aggregate(count_phone=Count('phone_number'))  #  Сколько авторов указали свой номер телефона?

        self.answer9 = Author.objects.filter(age__lt=25)  #  Какие авторы имеют возраст младше 25 лет?

        self.answer10 = Author.objects.annotate(count=Count('entries'))  #  Сколько статей написано каждым автором?
